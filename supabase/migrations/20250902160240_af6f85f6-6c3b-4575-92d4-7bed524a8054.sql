...
-- Add an additional policy to allow inserts from edge functions via RPC
-- This provides flexibility for future edge function implementations
CREATE POLICY "Allow edge function inserts via RPC" 
ON public.invitation_requests 
FOR INSERT 
WITH CHECK (
  -- Allow inserts when called from database functions (like from edge functions)
  current_setting('role', true) = 'service_role'
);

-- Log this security improvement
INSERT INTO public.invitation_security_log (event_type, details) 
VALUES (
  'security_policy_update', 
  '{"action": "restricted_invitation_requests_insert", "reason": "prevent_direct_table_access", "date": "' || now()::text || '"}'::jsonb
);