...
AS $$
  ...
  WITH board_users AS (
    -- Board creator
    SELECT b.created_by as user_id, 'owner' as relationship, 'owner' as role_name
    FROM public.boards b 
    WHERE b.id = _board_id
      AND public.can_access_board(auth.uid(), _board_id)
    
    UNION ALL
    
    -- Board members
    SELECT bm.user_id, 'member' as relationship, bm.role::text as role_name
    ...
    -- Board assignments
    SELECT ba.user_id, 'assigned' as relationship, 'volunteer' as role_name
